...
inline NumberResult parse_number(const std::string& input) {
    try {
        size_t pos = 0;
...
inline bool has_error(const NumberResult& result) {
    return !result.second.empty();
}

// Get the error message from a result
inline const std::string& get_error(const NumberResult& result) {
    return result.second;
}

// Get the value from a result (throws if there's an error)
inline int get_value(const NumberResult& result) {
    if (has_error(result)) {
